...
#pragma warning restore CA1825
        ...
        public static AsyncFlowControl SuppressFlow()
        {
            ...
            {
                throw new InvalidOperationException("Context flow is already suppressed.");
            }
        ...
        public static void RestoreFlow()
        {
            ...
            {
                throw new InvalidOperationException("Cannot restore context flow when it is not suppressed.");
            }
        ...
        {
            Debug.Assert(Thread.CurrentThread.IsThreadPoolThread); // there are no dedicated threadpool threads on runtimes where we can't start threads
        ...
        internal static void OnValuesChanged(ExecutionContext? previousExecutionCtx, ExecutionContext? nextExecutionCtx)
        {
            ...
            {
                Environment.FailFast("An exception was not handled in an AsyncLocal<T> notification callback.", ex);
            }
        }
        ...
        {
            throw new InvalidOperationException("Cannot call Set on a null context");
        }
    ...
    public struct AsyncFlowControl : IEquatable<AsyncFlowControl>, IDisposable
    {
        ...
        public void Undo()
        {
            if (_thread == null)
            {
                throw new InvalidOperationException("AsyncFlowControl object can be used only once to call Undo().");
            }
            if (Thread.CurrentThread != _thread)
            {
                throw new InvalidOperationException("AsyncFlowControl object must be used on the thread where it was created.");
            }
            ...
            {
                throw new InvalidOperationException("AsyncFlowControl objects can be used to restore flow only on a Context that had its flow suppressed.");
            }
