...
namespace System.Threading
{
    ...
    internal sealed class TimerQueueTimer : IThreadPoolWorkItem
    {
            ...
            lock (_associatedTimerQueue)
            {
                ...
                if (_canceled)
                {
                    if (notifyWhenNoCallbacksRunning is WaitHandle)
                    {
                        ...
                        var e = new InvalidOperationException("The Timer was already closed using an incompatible Dispose method.");
        ...
        internal void SignalNoCallbacksRunning()
        {
            ...
            {
                // Does this need to support other kinds of WaitHandles?
    ...
    public sealed class Timer : IDisposable, IAsyncDisposable
    {
        ...
        {
            if (dueTime < -1)
                throw new ArgumentOutOfRangeException(nameof(dueTime), ArgumentOutOfRangeException.NeedNonNegOrNegative1);
            if (period < -1)
                throw new ArgumentOutOfRangeException(nameof(period), ArgumentOutOfRangeException.NeedNonNegOrNegative1);
                     ...
                     TimeSpan period)
        {
            ...
            if (dueTm > MaxSupportedTimeout)
                throw new ArgumentOutOfRangeException(nameof(dueTime), ArgumentOutOfRangeException.TimeoutTooLarge);

            long periodTm = (long)period.TotalMilliseconds;
            if (periodTm < -1)
                throw new ArgumentOutOfRangeException(nameof(period), ArgumentOutOfRangeException.NeedNonNegOrNegative1);
            if (periodTm > MaxSupportedTimeout)
                throw new ArgumentOutOfRangeException(nameof(period), ArgumentOutOfRangeException.PeriodTooLarge);
