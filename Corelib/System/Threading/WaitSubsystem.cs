...
namespace System.Threading
{
    ...
    ///     interruptible
    ///   - <see cref="LowLevelLock"/> is used for the process-wide mutex <see cref="s_lock"/>
    ...
    ///   - Synchronization
    ///     - A process-wide mutex <see cref="s_lock"/> is used to synchronize most operations and the signal state of all
    ...
    ///       bypass the mutex and use interlocked operations for uncontended cases, but at the cost of making multi-waits more
    ...
    ///       mutex before checking the wait state of a thread and signaling the thread to wake up.
    ///
    ...
    ///   - <see cref="LowLevelLock"/>, used by the process-wide mutex <see cref="s_lock"/>, uses interlocked operations to
    ///     acquire and release the mutex when there is no need to wait or to release a waiter. This is significantly faster than
    ///     using <see cref="LowLevelMonitor"/> as a mutex, which uses pthread mutex functionality through p/invoke. The mutex is
    ///     typically not held for very long, especially since allocations inside the mutex will be rare.
    ...
    internal static partial class WaitSubsystem
    {
        ...
        {
            return WaitableObject.NewEvent(initiallySignaled, resetMode);
        }

        public static WaitableObject NewSemaphore(int initialSignalCount, int maximumSignalCount)
        {
            return WaitableObject.NewSemaphore(initialSignalCount, maximumSignalCount);
        }

        public static WaitableObject NewMutex(bool initiallyOwned)
        {
            ...
            // by the thread. See <see cref="ThreadWaitInfo.LockedMutexesHead"/>. So, acquire the mutex only after all
        ...
        public static WaitableObject? CreateNamedMutex(bool initiallyOwned, string name, out bool createdNew)
        {
        ...
        public static OpenExistingResult OpenNamedMutex(string name, out WaitableObject? result)
        {
            ...
            result = status == OpenExistingResult.Success ? mutex! : null;
            return status;
        }
        ...
        public static int Wait(
            Span<WaitableObject> waitHandles,
            bool waitForAll,
            int timeoutMilliseconds)
        {
            ...
            try
            {
                ...
                {
                    WaitableObject waitableObject = waitHandles[i];
                    if (waitForAll)
                    {
            ...
            bool prioritize = false)
        {
            ...
            try
            {
                ...
                {
                    throw new InvalidOperationException("The WaitHandle cannot be signaled because it would exceed its maximum count.", ex);
                }
                waitCalled = true;
            ...
            {
                // Once the wait function is called, it will release the mutex
                if (waitCalled)
                {
