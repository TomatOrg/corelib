...
namespace System.Text
{
    ...
    internal class EncoderNLS : Encoder
    {
        ...
        public override unsafe int GetByteCount(char[] chars, int index, int count, bool flush)
        {
            ...
            if (chars.Length - index < count)
                throw new ArgumentOutOfRangeException(nameof(chars),
                    ArgumentOutOfRangeException.IndexCountBuffer);
        ...
        internal override unsafe int GetBytes(char* chars, int charCount, byte* bytes, int byteCount, bool flush)
        {
            // Validate parameters
            if (chars == null || bytes == null)
                throw new ArgumentNullException(chars == null ? nameof(chars) : nameof(bytes),
        ...
        internal override unsafe void Convert(char* chars, int charCount,
        ...
        internal int DrainLeftoverDataForGetByteCount(ReadOnlySpan<char> chars, out int charsConsumed)
        {
            ...
            if (_fallbackBuffer != null && _fallbackBuffer.Remaining > 0)
            {
                throw new ArgumentException(
                    $"Must complete Convert() operation or call Encoder.Reset() before calling GetBytes() or GetByteCount(). Encoder '{Encoding.EncodingName}' fallback '{_fallbackBuffer.GetType()}'.");
            }
