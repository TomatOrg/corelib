...
*/
        ...
        internal static int IndexOf(ReadOnlySpan<char> source, ReadOnlySpan<char> value)
        {
            ...
            ref char pSourceLimit = ref Unsafe.Add(ref pSource, (source.Length - value.Length));
            ref char pValueLimit = ref Unsafe.Add(ref pValue, value.Length - 1);
            ref char pCurrentSource = ref pSource;

            while (!Unsafe.IsAddressGreaterThan(ref pCurrentSource, ref pSourceLimit))
            {
                ref char pVal = ref pValue;
                ref char pSrc = ref pCurrentSource;

                while (!Unsafe.IsAddressGreaterThan(ref pVal, ref pValueLimit))
                {
                    if (!char.IsHighSurrogate(pVal) || Unsafe.AreSame(ref pVal, ref pValueLimit))
                    {
                        if (pVal != pSrc && ToUpper(pVal) != ToUpper(pSrc))
                            break; // no match

                        pSrc = ref Unsafe.Add(ref pSrc, 1);
                        pVal = ref Unsafe.Add(ref pVal, 1);
                        continue;
                    }

                    if (char.IsHighSurrogate(pSrc) && char.IsLowSurrogate(Unsafe.Add(ref pSrc, 1)) && char.IsLowSurrogate(Unsafe.Add(ref pVal, 1)))
                    {
                        // Well formed surrogates
                        // both the source and the Value have well-formed surrogates.
                        if (!SurrogateCasing.Equal(pSrc, Unsafe.Add(ref pSrc, 1), pVal, Unsafe.Add(ref pVal, 1)))
                            break; // no match

                        pSrc = ref Unsafe.Add(ref pSrc, 2);
                        pVal = ref Unsafe.Add(ref pVal, 2);
                        continue;
                    }
                ...
                if (Unsafe.IsAddressGreaterThan(ref pVal, ref pValueLimit))
                {
                    // Found match.
                    return (int)((Unsafe.ByteOffset(ref pSource, ref pCurrentSource)) / sizeof(char));
                }

                pCurrentSource = ref Unsafe.Add(ref pCurrentSource, 1);
            }
        ...
        internal static unsafe int LastIndexOf(ReadOnlySpan<char> source, ReadOnlySpan<char> value)
        {
            ...
            ref char pValueLimit = ref Unsafe.Add(ref pValue, value.Length - 1);
            ref char pCurrentSource = ref Unsafe.Add(ref pSource, (source.Length - value.Length));
