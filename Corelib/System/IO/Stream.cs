...
#pragma warning restore CS8774
        ...
        {
            get => throw new InvalidOperationException(InvalidOperationException.TimeoutsNotSupported);
            set => throw new InvalidOperationException(InvalidOperationException.TimeoutsNotSupported);
        }
            ...
            byte[] buffer, int offset, int count)
        {
            ...
            var task = new ReadWriteTask(true, delegate
            {
        ...
        public virtual Task<int> ReadAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken) =>
            cancellationToken.IsCancellationRequested ?
                ...
                BeginReadInternal(buffer, offset, count);
        ...
        internal Task BeginWriteInternal(byte[] buffer, int offset, int count)
        {
            ValidateBufferArguments(buffer, offset, count);
            if (!CanWrite)
            {
            ...
            // thread if it does a second IO request until the first one completes.
            SemaphoreSlim semaphore = EnsureAsyncActiveSemaphoreInitialized();
            Task? semaphoreTask = semaphore.WaitAsync(); // kick off the asynchronous wait, but don't block

            // Create the task to asynchronously do a Write.  This task serves both
            // as the asynchronous work item and as the IAsyncResult returned to the user.
            var task = new ReadWriteTask(false, delegate
            {
        ...
        private sealed class ReadWriteTask : Task<int>
        {
            ...
            public ReadWriteTask(
                ...
                base(function, null, CancellationToken.None, TaskCreationOptions.DenyChildAttach)
            {
        ...
        public virtual Task WriteAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken) =>
            ...
            cancellationToken.IsCancellationRequested ?
                ...
                BeginWriteInternal(buffer, offset, count);
        ...
        public virtual int Read(Span<byte> buffer)
        {
            ...
            try
            {
                ...
                {
                    throw new IOException(IOException.StreamTooLong);
                }
        ...
        protected static void ValidateCopyToArguments(Stream destination, int bufferSize)
        {
            ...
            if (bufferSize <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(bufferSize), bufferSize, ArgumentOutOfRangeException.NeedPosNum);
            }
