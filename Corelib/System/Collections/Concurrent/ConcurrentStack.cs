...
namespace System.Collections.Concurrent
{
    ...
    public class ConcurrentStack<T> : IProducerConsumerCollection<T>, IReadOnlyCollection<T>
    {
        ...
        private static void ValidatePushPopRangeInput(T[] items, int startIndex, int count)
        {
            ...
            if (count < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(count), "The count argument must be greater than or equal to zero.");
            }
            int length = items.Length;
            if (startIndex >= length || startIndex < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(startIndex), "The startIndex argument must be greater than or equal to zero.");
            }
