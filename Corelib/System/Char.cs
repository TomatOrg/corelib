...
namespace System
{
    public readonly struct Char : IComparable<char>, IEquatable<char>, ISpanFormattable
    {
        //
        // Member Variables
        //
        ...
        **This static methods takes a character and returns the String representation of it.
        ...
        public static char Parse(string s)
        {
            ...
            if (s.Length != 1)
            {
                throw new FormatException(FormatException.NeedSingleChar);
            }
            return s[0];
        }
        ...
        ** Check if the specified UnicodeCategory belongs to the separator categories.
        ...
        public static string ConvertFromUtf32(int utf32)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(utf32), ArgumentOutOfRangeException.InvalidUTF32);
            }
        ...
        private static void ConvertToUtf32_ThrowInvalidArgs(uint highSurrogateOffset)
        {
            ...
            if (highSurrogateOffset > CharUnicodeInfo.HIGH_SURROGATE_RANGE)
            {
                throw new ArgumentOutOfRangeException(
                    ...
                    message: ArgumentOutOfRangeException.InvalidHighSurrogate);
            }
            else
            {
                throw new ArgumentOutOfRangeException(
                    paramName: "lowSurrogate",
                    message: ArgumentOutOfRangeException.InvalidLowSurrogate);
            }
        }
            ...
            if (temp1 >= 0 && temp1 <= 0x7ff)
            {
                ...
                if (temp1 <= 0x3ff)
                {
                    ...
                    if (index < s.Length - 1)
                    {
                        ...
                        {
                            throw new ArgumentException($"Found a high surrogate char without a following low surrogate at index: {index}. The input may not be in this encoding, or may not contain valid Unicode (UTF-16) characters.", nameof(s));
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    throw new ArgumentException($"Found a low surrogate char without a preceding high surrogate at index: {index}. The input may not be in this encoding, or may not contain valid Unicode (UTF-16) characters.", nameof(s));
                }
            }
